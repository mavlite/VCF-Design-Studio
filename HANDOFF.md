...
- **Test suite:** **2197 unit + 60 migration + 46 snapshot + 44
  ...
  defensive coverage). The M2.1 **round-trip matrix**
  (`tests/unit/round-trip-matrix.test.js` + helpers) adds whole-model
  JSON + CSV round-trip verification with a coverage meta-guard (see
  Test-coverage debt below); `round-trip-depth.test.js` was folded into
  it. engine.js coverage: **93.5 / 83.6 / 99.5 / 97.8**
  (stmts/branches/funcs/lines), above the 95/95/75/90 gate.
...
- **Round-trip coverage matrix — landed (M2.1, 2026-05-29).**
  `tests/unit/round-trip-matrix.test.js` drives one kitchen-sink fleet
  (`tests/helpers/kitchen-sink-fleet.js` — every factory exercised,
  value-bearing fields populated; `tests/helpers/sentinel-walk.js`
  stamps each leaf with a unique type-aware sentinel, with valid enum
  overrides) through BOTH round-trip paths: JSON save/load
  (`migrateFleet`) and the CSV cell-map (9.0 + 9.1). A **meta-guard**
  asserts every value-bearing leaf is either CSV-mapped, in
  `KNOWN_CSV_GAPS`, or matched by `NON_WORKBOOK_ALLOWLIST` (with a
  `why:`) — so a newly-added field that escapes coverage fails CI. A
  self-test proves the guard is live. `round-trip-depth.test.js` (themes
  11/12/13) was folded in. CSV-mapped surface: 378 paths on 9.0, 453 on
  9.1; JSON layer verifies the full surface.
