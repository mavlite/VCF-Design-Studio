...
function createPortgroupSlot() {
  return {
    ...
    uplink1: "Active",
    uplink2: "Active",
    vdsSlot: "",   // "Network Traffic: X" cell — the vDS/network this PG's traffic uses
  };
}
...
function createClusterNetworks() {
  return {
    ...
    vmotion: { vlan: null, subnet: null, gateway: null, pool: { start: null, end: null }, mtu: MTU_VMOTION, ipScheme: "IPv4", ipAssignment: "Static", ipv6: createNetworkIpv6() },
    vsan:    { vlan: null, subnet: null, gateway: null, pool: { start: null, end: null }, mtu: MTU_VSAN, ipScheme: "IPv4", ipAssignment: "Static", ipv6: createNetworkIpv6() },
    hostTep: { vlan: null, subnet: null, gateway: null, pool: { start: null, end: null }, mtu: MTU_TEP_RECOMMENDED, useDhcp: false, ipScheme: "IPv4", ipAssignment: "Static", ipv6: createNetworkIpv6() },
    edgeTep: { vlan: null, subnet: null, gateway: null, pool: { start: null, end: null }, mtu: MTU_TEP_RECOMMENDED, ipScheme: "IPv4", ipAssignment: "Static", ipv6: createNetworkIpv6() },
...
function createEdgeCluster() {
  return {
    ...
    // Static-IP-list network params (used when tepIpAllocation === "Static IP
    // List"). Node-1 input cells; Node-2 + Management Gateway are workbook
    // formulas (not mapped). tepStaticCidr exists only in the 9.1 workbook.
...
// One NSX IP-block pool definition (VCF 9.1). All five sub-fields map to the
// Configure Mgmt/WLD pool block; `ipBlocks` also has a flat 9.0 home, the
// other four are 9.1-only. `visibility` is a free string (the workbook cell
// carries no data-validation).
...
function createClusterVpcConfig() {
  return {
    ...
    tgwPool: createVpcIpBlockPool(),        // "Private - Transit Gateway IP Blocks" pool
  };
}
...
function createFleetAdConfig() {
  return {
    ...
    ca: {
      ...
      csrSubject: {
        commonName: "",                  // CN — usually the SDDC Mgr cert FQDN
        org: "",                         // e.g. "Rainpole"
        ou: "",                          // e.g. "IT"
...
function emitWorkbookCellMap(fleet, fleetResult, options) {
  ...
  for (const entry of WORKBOOK_CELL_MAP) {
    ...
    for (const ctx of contexts) {
      // Stamp the emit version on the iteration ctx so resolve closures
      // can pick version-specific dropdown values (e.g. _deployNetwork-
      // Block's ipAssignment, where 9.0 uses "IP Pool" and 9.1 may use
      // "Static IP Pool"). All iterations within a single emit call
      // share the same version, so direct mutation is safe.
      ctx.workbookVersion = version;
...
function _resolveXLSX() {
  ...
  // why: window.XLSX is the browser-bundle path (build-html.mjs inlines SheetJS); never set in Node/JSDOM tests.
  if (typeof window !== "undefined" && window.XLSX) return window.XLSX;
  /* istanbul ignore next */
  // why: globalThis.XLSX is the browser-bundler global-injection path; never set in the Node test environment.
  if (typeof globalThis !== "undefined" && globalThis.XLSX) return globalThis.XLSX;
  if (typeof require === "function") {
    // why: xlsx is always installed as a devDependency; require("xlsx") never throws in the test environment.
    /* istanbul ignore next */
...
function emitWorkbookXlsx(fleet, fleetResult, pristineWorkbookInput, options) {
  ...
  /* istanbul ignore next */
  // why: Blob + window are browser APIs; the Node test env returns buf (Uint8Array) unconditionally.
...
function emitWorkbookXlsxWithPasswords(fleet, fleetResult, pristineWorkbookInput, options) {
  ...
  const buf = XLSX.write(wb, { type: "array", bookType: "xlsx" });
  /* istanbul ignore next */
  // why: Blob + window are browser APIs; the Node test env always takes the `: buf` branch.
...
function importWorkbookCellMap(rows, options) {
  ...
  // Infer stretched placement on import (issue #112). AZ2 network config only
  // emits for stretched clusters, and every AZ2 resolve/apply is gated by
  // _isStretchedCtx (domain.placement === "stretched"). `placement` is NOT a
  // workbook field, so the draft's default "local" placement would make every
  // AZ2 apply a no-op and silently drop the values. If any non-empty AZ2-network
  // row is present, mark the owning domain stretched BEFORE the apply loop. A
  // non-stretched export emits AZ2 cells empty, so this never false-positives.
  const _mgmtDomain = draft.instances[0].domains.find((d) => d.type === "mgmt");
  const _wldDomain = draft.instances[0].domains.find((d) => d.type === "workload");
  for (const r of rows) {
    if (r.value === "" || r.value === null || r.value === undefined) continue;
    const e = _findCellMapEntry(r.sheet, r.cell, version);
    if (!e || !e.az2Network) continue;
    if ((e.scope === "mgmt-cluster" || e.scope === "initial-instance-mgmt-cluster") && _mgmtDomain) {
      _mgmtDomain.placement = "stretched";
    } else if ((e.scope === "workload-cluster" || e.scope === "additional-cluster") && _wldDomain) {
      _wldDomain.placement = "stretched";
    }
  }
  ...
  for (const row of sortedRows) {
    ...
    // why: additionalClusterCount is derived from the rows themselves so the cluster skeleton count
    //      always matches the per-cell cursor maximum; for all other scopes, newFleet() guarantees
    //      at least one context is returned by _iterateScope.
    /* istanbul ignore next */
    if (!ctx) {
...
const PASSWORD_POLICY = {
  ...
  // Theme O remainder — MS-CA bind credentials. The Certificate
  // Authority's admin user/password pair the workbook uses for CSR
  // submission (D84/D85 on Configure Mgmt 9.1, D83/D84 on 9.0). User-
  // supplied; VCF cannot auto-generate (external system).
  "ca-bind":               { len: 16, classes: { upper: 4, lower: 4, digit: 4, special: 4 }, alphabet: { special: _SPECIAL_SAFE } },
...
function _resolveCrypto() {
  ...
  /* istanbul ignore next */
  // why: window.crypto is the browser-only fallback; Node 16+ always returns via globalThis.crypto above.
...
function generateWorkbookVault(fleet, options) {
  ...
  const CAMP_B_KINDS = new Set([
    ...
    // bind account, Certificate Authority admin) that the user
    // controls. Without these in Camp B, generateWorkbookVault({
    // scope: "camp-b" }) silently skipped them.
...
function _portgroupSlotEntries({ scope, sheet, slotKey, slotLabel, cells, cells90 }) {
  ...
  // When cells90 is supplied, every entry gets cellByVersion routing +
  // workbookVersions covering both 9.0 and 9.1. Without it, the slot
  // stays 9.1-only.
  //
  // Defensive guard: if cells90 is provided but a field key is missing
  // (typo, copy-paste error), throw at construction. The alternative —
  // a silently-undefined cellByVersion["9.0"] — would route 9.0 emit to
  // the 9.1 cell (since fallback to top-level `cell` is 9.1), stamping
  // the wrong row on customer workbooks. Fail-fast at module load is
  // cheaper than a silent stamping bug.
  const dual = cells90 != null;
  if (dual) {
    for (const k of ["name", "lb", "uplink1", "uplink2", "networkTraffic"]) {
      if (typeof cells90[k] !== "string") {
        throw new Error(`_portgroupSlotEntries: cells90.${k} is required when cells90 is supplied (slot=${slotKey}, sheet=${sheet})`);
      }
    }
  }
  ...
  return tagCapability("portgroups", [
    {
      ...
      sheet, cell: cells.networkTraffic, ...versioned("networkTraffic"),
      label: `${slotLabel} Network Traffic`,
      verifyLabel: "Network Traffic",
      scope,
      resolve: (f, ctx) => _getPortgroup(ctx, slotKey).vdsSlot || "",
      apply: (f, ctx, v) => { _ensurePortgroup(ctx, slotKey).vdsSlot = String(v || ""); },
    },
    {
      sheet, cell: cells.name, ...versioned("name"),
    ...
    {
      sheet, cell: cells.lb, ...versioned("lb"),
    ...
    {
      sheet, cell: cells.uplink1, ...versioned("uplink1"),
    ...
    {
      sheet, cell: cells.uplink2, ...versioned("uplink2"),
...
//
// When `cells90` is provided, every entry gets a cellByVersion override
// routing 9.0 to its row addresses and 9.1 to the (default) `cells`
// addresses, and `workbookVersions` covers both. Without `cells90` the
// block stays 9.1-only.
function _nsxHostOverlayBlockEntries({ scope, sheet, cells, cells90, teamingPolicyLabel, activeUplinkEnum }) {
  ...
  const dualVersion = cells90 != null;
  // Defensive guard: when dualVersion, every entry MUST have a valid
  // cellKey resolving to a 9.0 cell address. A missing/typo'd cellKey
  // would produce `cellByVersion["9.0"] = undefined` and fall back to
  // the 9.1 cell address — silently stamping the wrong row on a 9.0
  // workbook export. Fail-fast at module load instead.
  const E = (cell, label, verifyLabel, field, opts = {}) => {
    const c90 = dualVersion ? cells90[opts.cellKey] : null;
    if (dualVersion && (typeof c90 !== "string" || !opts.cellKey)) {
      throw new Error(`_nsxHostOverlayBlockEntries: cells90.${opts.cellKey || "<missing cellKey>"} is required when cells90 is supplied (sheet=${sheet}, field=${field})`);
    }
    return {
      sheet, cell, label, verifyLabel,
      ...(dualVersion && c90 ? { cellByVersion: { "9.0": c90, "9.1": cell } } : {}),
      workbookVersions: dualVersion ? ["9.0", "9.1"] : ["9.1"],
      scope,
  ...
  const entries = [
    ...
    E(cells.tzName, "NSX Transport Zone Name", "Transport Zone Name", "transportZoneName", { cellKey: "tzName" }),
    E(cells.vlan, "NSX Host Overlay VLAN", "VLAN ID", "vlan", { cellKey: "vlan" }),
    E(cells.ipAssignment, "NSX TEP IP Assignment", "IP Assignment", "ipAssignment", {
      cellKey: "ipAssignment",
    ...
    E(cells.poolName, "NSX Host Overlay Pool Name", "Pool name", "poolName", { cellKey: "poolName" }),
    E(cells.poolDescription, "NSX Host Overlay Pool Description", "Description", "poolDescription", { cellKey: "poolDescription" }),
    E(cells.cidr, "NSX Host Overlay CIDR", "CIDR", "cidr", { cellKey: "cidr" }),
    E(cells.rangeStart, "NSX Host Overlay IP Range Start", "IP Range Start", "ipRangeStart", { cellKey: "rangeStart" }),
    E(cells.rangeEnd, "NSX Host Overlay IP Range End", "IP Range End", "ipRangeEnd", { cellKey: "rangeEnd" }),
    E(cells.gatewayIp, "NSX Host Overlay Gateway IP", "Gateway IP", "gatewayIp", { cellKey: "gatewayIp" }),
    E(cells.vlanTzName, "NSX VLAN Transport Zone Name", "VLAN Transport Zone Name", "vlanTransportZoneName", { cellKey: "vlanTzName" }),
    E(cells.uplinkProfileName, "NSX Uplink Profile Name", "NSX Uplink Profile Name", "uplinkProfileName", { cellKey: "uplinkProfileName" }),
    E(cells.numberOfUplinks, "NSX Number of Uplinks", "Number of NSX uplinks", "numberOfUplinks", { cellKey: "numberOfUplinks" }),
    E(cells.uplink1Name, "NSX Uplink 1 Name", "uplink-1", "uplinkName1", { cellKey: "uplink1Name" }),
    E(cells.uplink2Name, "NSX Uplink 2 Name", "uplink-2", "uplinkName2", { cellKey: "uplink2Name" }),
    E(cells.hostOverlayProfile, "NSX Host Overlay Profile Name", "NSX Host Overlay Network Profile Name", "hostOverlayProfileName", { cellKey: "hostOverlayProfile" }),
    E(cells.teamingPolicy, "NSX Teaming Policy", teamingPolicyLabel, "teamingPolicy", {
      cellKey: "teamingPolicy",
...
// Emit the 5 cell-map entries for one NSX IP-block pool (External or Private
// TGW) on a Configure sheet. `ipBlocks` is dual-version ({v90,v91}); the other
// four sub-fields are 9.1-only. Mirrors _supervisorConfigureBlockEntries.
function _vpcPoolBlockEntries({ scope, sheet, poolKey, poolLabel, scopeShort, ipBlocksVerifyLabel, cells }) {
  const getPool = (ctx) => _getVpcConfig(ctx)[poolKey] || createVpcIpBlockPool();
  const ensurePool = (ctx) => {
    const vpc = _ensureVpcConfig(ctx);
    if (!vpc) return null;
    if (!vpc[poolKey] || typeof vpc[poolKey] !== "object") vpc[poolKey] = createVpcIpBlockPool();
    return vpc[poolKey];
  };
  const E = (cell, fieldLabel, verifyLabel, field) => {
    const dual = cell && typeof cell === "object";
    const base = dual
      ? { cell: cell.v91, cellByVersion: { "9.0": cell.v90, "9.1": cell.v91 }, workbookVersions: ["9.0", "9.1"] }
      : { cell, workbookVersions: ["9.1"] };
    return {
      sheet, ...base,
      label: `${poolLabel} — ${fieldLabel} (${scopeShort})`,
      verifyLabel, scope,
      resolve: (f, ctx) => getPool(ctx)[field] || "",
...
function _supervisorConfigureBlockEntries({ scope, sheet, cells, edgeClusterEnum }) {
  ...
  // Theme N — `cell` may be either a string (9.1-only) or an object
  // { v90, v91 } (dual-version). The 9.0 workbook had a smaller
  // Supervisor block (~9 fields: Version, Edge Cluster, Admin Password,
  // Node 1/2/3 IPs, Cluster VIP/FQDN/Name) — those 9 fields are
  // dual-version; everything else 9.1-only because the 9.0 workbook
  // didn't carry the concept.
  const expandCell = (cell) => {
    if (typeof cell === "string") {
      return { cellProp: { cell }, versions: ["9.1"] };
    }
    return {
      cellProp: { cell: cell.v90, cellByVersion: { "9.1": cell.v91 } },
      versions: ["9.0", "9.1"],
    };
  };
  const E = (cell, label, verifyLabel, field, opts = {}) => {
    const { cellProp, versions } = expandCell(cell);
    return {
      sheet, ...cellProp, label, verifyLabel,
  ...
  const { cellProp: vaultCellProp, versions: vaultVersions } = expandCell(cells.adminPassword);
  const vaultPwd = {
    sheet, ...vaultCellProp,
    label: `Supervisor Admin Password (${scope === "mgmt-cluster" ? "Mgmt" : "WLD"})`,
    verifyLabel: "Admin Password",
    workbookVersions: vaultVersions,
    scope,
...
function _az2NetworkConfigEntries(scope, sheet, networkKey, displayName, cells, shape) {
  ...
  const E = (cell90, cell91, label, verifyLabel, resolve, apply, extra = {}) => {
    if (!cell90 && !cell91) return null;
    // az2Network: true marks every AZ2 network-config entry. resolve/apply are
    // gated by _isStretchedCtx; importWorkbookCellMap uses this flag to infer
    // stretched placement on import (issue #112) so the apply rows don't no-op.
    const entry = { sheet, label, verifyLabel, workbookVersions: [], scope, resolve, az2Network: true, ...extra };
    if (cell90) { entry.cell = cell90; entry.workbookVersions.push("9.0"); }
    if (cell91) {
      if (!entry.cell) entry.cell = cell91;
...
function _deployNetworkBlock(scope, sheet, networkKey, displayName, cells) {
  ...
  // IP Assignment (Static | DHCP | SLAAC) — 9.1-only, per network pool on
  // Deploy WLD/Cluster. SLAAC is IPv6 stateless autoconfig. NOTE: a distinct
  // key from `ipAssignment91`, which is the existing useDhcp→"IP Assignment
  // (TEP)" dropdown below — different concept, different cell.
  if (cells.ipAssignMode91) {
    out.push(E(null, cells.ipAssignMode91, `${cap} IP Assignment`, "IP Assignment",
      (f, ctx) => _getClusterNetwork(ctx, networkKey).ipAssignment || "Static",
      (f, ctx, v) => { _ensureClusterNetwork(ctx, networkKey).ipAssignment = ["Static", "DHCP", "SLAAC"].includes(v) ? v : "Static"; },
      { dataValidation: ["Static", "DHCP", "SLAAC"] }));
  }
  ...
  if (cells.ipAssignment90 || cells.ipAssignment91) {
    ...
    out.push(E(cells.ipAssignment90, cells.ipAssignment91, `${cap} IP Assignment`, cells.ipAssignmentVerifyLabel || "IP Assignment (TEP)",
      (f, ctx) => {
        ...
        // Pick the static label by the emit version that the emitter
        // stamped on ctx. Fall back to inferring from which cells are
        // mapped when ctx.workbookVersion is absent (e.g. a unit test
        // invoking resolve directly without going through emit).
        if (ctx && ctx.workbookVersion === "9.1") return staticLabel91;
        if (ctx && ctx.workbookVersion === "9.0") return staticLabel90;
...
function _edgeAllocationEntries(scope, sheet, scopeShort, cells, tepAllocDv) {
  ...
  // Plain-text entry (no dropdown). `cell` is a {v90,v91} pair (dual-version)
  // or a string (9.1-only).
  const T = (cell, field, label, verifyLabel) => {
    ...
    const base = dual
      ...
      : { cell, workbookVersions: ["9.1"] };
    return {
      sheet, ...base, label, verifyLabel, scope,
...
const WORKBOOK_CELL_MAP = [
  ...
  {
    // 9.1-only: Data-in-Transit encryption ON/OFF toggle. The pristine
    // workbook ships with "Selected" at D215, so the factory default is
    // enabled=true. The mgmt cluster has its own equivalent at Deploy
    // Management Domain L59 (see the entry just below).
  ...
  {
    // 9.1-only mgmt-cluster Data-in-Transit encryption toggle (coverage
    // sweep). Deploy Management Domain L59 "Activate vSAN Data-in-Transit
    // encryption" (Selected/Unselected), verified against the pristine 9.1
    // fixture. Mirrors the WLD entry above; reads the same model field.
    sheet: "Deploy Management Domain", cell: "L59",
    label: "Mgmt DIT Encryption Enabled",
    verifyLabel: "Data-in-Transit encryption",
    workbookVersions: ["9.1"],
    scope: "mgmt-cluster",
    dataValidation: ["Selected", "Unselected"],
  ...
  // ─── Theme L (option a) — Per-host FQDN tables (Deploy WLD + Deploy Cluster)
  // The mgmt-cluster FQDN block above (L82/L128 on Deploy Mgmt) is the
  // only per-host FQDN table Theme 14 originally stamped. The workbook
  // also carries the same 16-host FQDN table on Deploy WLD (D120 9.0 /
  // D131 9.1) and Deploy Cluster (D84 9.0 / D96 9.1). Same model:
  // resolveHostname() + fleet DNS suffix. Same round-trip: strip the
  // DNS suffix and persist as cluster.hostOverrides[i].hostname.
  //
  // Cells verified against test-fixtures/workbook/workbook-cell-meta-
  // {9.0,9.1}.json (label "Host 1" + 15 siblings, dataType "n").
  {
    sheet: "Deploy Workload Domain",
    cellPattern: "D{120+i}",
    cellPatternByVersion: { "9.1": "D{131+i}" },
    label: "WLD Host #{i+1} FQDN",
    // Workbook labels these cells generically as "Host 1" through
    // "Host 16" (vs the more specific "Host #1 FQDN" used on Deploy
    // Mgmt's FQDN block). Anchor verify to the workbook's actual label.
    verifyLabel: "Host 1",
    workbookVersions: ["9.0", "9.1"],
  ...
  },
  {
    ...
    cellPattern: "D{84+i}",
    cellPatternByVersion: { "9.1": "D{96+i}" },
    label: "Additional Cluster Host #{i+1} FQDN",
    verifyLabel: "Host 1",
    workbookVersions: ["9.0", "9.1"],
  ...
  // ─── Theme O (sub-themes 2a + 2c) — Witness + Deploy Cluster DNS/NTP refs
  //
  // The witness host block on Configure Mgmt (D385-D398, 9.1) and the
  // per-cluster section on Deploy Cluster (D373-D378, 9.1) carry DNS
  // Domain / DNS Servers / NTP Servers cells that the studio's
  // fleet-level networkConfig already models. These cells echo the
  // fleet's DNS/NTP config — no new model needed.
  //
  // 9.1-only: the 9.0 workbook's equivalent rows are mostly formula
  // cells (only D397 on 9.0 Configure Mgmt is an input, but with a
  // different label "Name" — different field entirely).
  //
  // Sub-theme 2b (NSX GM Node 2/3 search lists at D485/D502) is
  // deliberately deferred — it conflicts with Theme 9's documented
  ...
  {
    sheet: "Deploy Cluster", cell: "D377",
    label: "Additional Cluster NTP Server #1",
  ...
  {
    sheet: "Deploy Cluster", cell: "D378",
    label: "Additional Cluster NTP Server #2",
    verifyLabel: "NTP Servers",
    workbookVersions: ["9.1"],
    scope: "additional-cluster",
    resolve: (f) => (f.networkConfig && f.networkConfig.ntp && f.networkConfig.ntp.servers && f.networkConfig.ntp.servers[1]) || "",
  ...
  // Most entries are 9.1-only — those workbook cells didn't exist in 9.0.
  // The Load Balancer FQDN at L161 (9.1) DOES exist in 9.0 at L63 with a
  // matching label; it's pulled out below as a dual-version entry.
  ...[
  ...
  // VCF Ops Load Balancer FQDN — dual-version. The other VCF Ops nodes
  // exist only in 9.1 (load-balanced cluster topology); the standalone
  // appliance FQDN ("Load Balancer FQDN" label) is the one common cell.
  {
    sheet: "Deploy Management Domain", cell: "L161", cellByVersion: { "9.0": "L63", "9.1": "L161" },
    label: "VCF Ops Load Balancer FQDN",
    verifyLabel: "Load Balancer FQDN",
    workbookVersions: ["9.0", "9.1"],
    scope: "initial-instance-mgmt-cluster",
    emitOnly: true,
    resolve: (fleet, ctx) => {
      const inst = ctx.instance;
      const dn = (fleet.networkConfig && fleet.networkConfig.dns && fleet.networkConfig.dns.primaryDomain) || "";
      const base = inst && inst.name ? `${inst.name}-ops01` : "vcf-ops01";
      return dn ? `${base}.${dn}` : base;
    },
  },
  ...
  {
    sheet: "Deploy Management Domain", cell: "L182", cellByVersion: { "9.0": "L93", "9.1": "L182" },
  ...
  {
    sheet: "Deploy Management Domain", cell: "L185", cellByVersion: { "9.0": "L96", "9.1": "L185" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D156", cellByVersion: { "9.0": "D153", "9.1": "D156" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D157", cellByVersion: { "9.0": "D154", "9.1": "D157" },
  ...
  {
    sheet: "Configure Workload Domain", cell: "D99", cellByVersion: { "9.0": "D96", "9.1": "D99" },
  ...
  {
    sheet: "Configure Workload Domain", cell: "D100", cellByVersion: { "9.0": "D97", "9.1": "D100" },
  ...
  //
  // Theme 9 originally treated searchList as a workbook-formula default
  // and didn't export it. The 9.1 workbook actually has 2 of the 3
  // search list cells as user-input slots (Node 3's D519 is still a
  // formula); the Node 1 and Node 2 slots accept user values. Resolved
  // by stamping just the user-input cells.
  //
  ...
  {
    sheet: "Configure Management Domain", cell: "D312", cellByVersion: { "9.1": "D383" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D313", cellByVersion: { "9.1": "D384" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D315", cellByVersion: { "9.1": "D386" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D316", cellByVersion: { "9.1": "D387" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D319", cellByVersion: { "9.1": "D390" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D371", cellByVersion: { "9.1": "D442" },
    label: "Witness FQDN",
  ...
  // password cells).
  {
    sheet: "Configure Management Domain", cell: "D318", cellByVersion: { "9.1": "D389" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D361", cellByVersion: { "9.1": "D432" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D362", cellByVersion: { "9.1": "D433" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D363", cellByVersion: { "9.1": "D434" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D364", cellByVersion: { "9.1": "D435" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D365", cellByVersion: { "9.1": "D436" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D366", cellByVersion: { "9.1": "D437" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D367", cellByVersion: { "9.1": "D438" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D368", cellByVersion: { "9.1": "D439" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D369", cellByVersion: { "9.1": "D440" },
  ...
  {
    sheet: "Configure Management Domain", cell: "D370", cellByVersion: { "9.1": "D441" },
  ...
  // 9.0 + 9.1 row addresses both confirmed against pristine workbook
  // cell-meta — labels + dataValidation enums match across versions.
  ..._portgroupSlotEntries({ scope: "mgmt-cluster", sheet: "Deploy Management Domain", slotKey: "mgmt", slotLabel: "Mgmt PG (Deploy Mgmt: ESX Mgmt)",
    cells:   { networkTraffic: "L238", name: "L239", lb: "L240", uplink1: "L241", uplink2: "L242" },
    cells90: { networkTraffic: "L220", name: "L221", lb: "L222", uplink1: "L223", uplink2: "L224" } }),
  ..._portgroupSlotEntries({ scope: "mgmt-cluster", sheet: "Deploy Management Domain", slotKey: "vmMgmt", slotLabel: "VM Mgmt PG (Deploy Mgmt)",
    cells:   { networkTraffic: "L243", name: "L244", lb: "L245", uplink1: "L246", uplink2: "L247" },
    cells90: { networkTraffic: "L225", name: "L226", lb: "L227", uplink1: "L228", uplink2: "L229" } }),
  ..._portgroupSlotEntries({ scope: "mgmt-cluster", sheet: "Deploy Management Domain", slotKey: "vmotion", slotLabel: "vMotion PG (Deploy Mgmt)",
    cells:   { networkTraffic: "L253", name: "L254", lb: "L255", uplink1: "L256", uplink2: "L257" },
    cells90: { networkTraffic: "L230", name: "L231", lb: "L232", uplink1: "L233", uplink2: "L234" } }),
  ..._portgroupSlotEntries({ scope: "mgmt-cluster", sheet: "Deploy Management Domain", slotKey: "vsan", slotLabel: "vSAN PG (Deploy Mgmt)",
    cells:   { networkTraffic: "L258", name: "L259", lb: "L260", uplink1: "L261", uplink2: "L262" },
    cells90: { networkTraffic: "L235", name: "L236", lb: "L237", uplink1: "L238", uplink2: "L239" } }),
  ..._portgroupSlotEntries({ scope: "mgmt-cluster", sheet: "Deploy Management Domain", slotKey: "nfs", slotLabel: "NFS PG (Deploy Mgmt)",
    cells:   { networkTraffic: "L263", name: "L264", lb: "L265", uplink1: "L266", uplink2: "L267" },
    cells90: { networkTraffic: "L240", name: "L241", lb: "L242", uplink1: "L243", uplink2: "L244" } }),
  ...
  ..._portgroupSlotEntries({ scope: "workload-cluster", sheet: "Deploy Workload Domain", slotKey: "mgmt", slotLabel: "Mgmt PG (Deploy WLD)",
    cells:   { networkTraffic: "D286", name: "D287", lb: "D288", uplink1: "D289", uplink2: "D290" },
    cells90: { networkTraffic: "D271", name: "D272", lb: "D273", uplink1: "D274", uplink2: "D275" } }),
  ..._portgroupSlotEntries({ scope: "workload-cluster", sheet: "Deploy Workload Domain", slotKey: "vmMgmt", slotLabel: "VM Mgmt PG (Deploy WLD)",
    cells:   { networkTraffic: "D291", name: "D292", lb: "D293", uplink1: "D294", uplink2: "D295" },
    cells90: { networkTraffic: "D276", name: "D277", lb: "D278", uplink1: "D279", uplink2: "D280" } }),
  ..._portgroupSlotEntries({ scope: "workload-cluster", sheet: "Deploy Workload Domain", slotKey: "vmotion", slotLabel: "vMotion PG (Deploy WLD)",
    cells:   { networkTraffic: "D296", name: "D297", lb: "D298", uplink1: "D299", uplink2: "D300" },
    cells90: { networkTraffic: "D281", name: "D282", lb: "D283", uplink1: "D284", uplink2: "D285" } }),
  ..._portgroupSlotEntries({ scope: "workload-cluster", sheet: "Deploy Workload Domain", slotKey: "principalStorage", slotLabel: "Principal Storage PG (Deploy WLD)",
    cells:   { networkTraffic: "D301", name: "D302", lb: "D303", uplink1: "D304", uplink2: "D305" },
    cells90: { networkTraffic: "D286", name: "D287", lb: "D288", uplink1: "D289", uplink2: "D290" } }),
  ..._portgroupSlotEntries({ scope: "workload-cluster", sheet: "Deploy Workload Domain", slotKey: "vsanStorageClient", slotLabel: "vSAN Storage Client PG (Deploy WLD)",
    cells:   { networkTraffic: "D306", name: "D307", lb: "D308", uplink1: "D309", uplink2: "D310" },
    cells90: { networkTraffic: "D291", name: "D292", lb: "D293", uplink1: "D294", uplink2: "D295" } }),
  ...
  ..._portgroupSlotEntries({ scope: "additional-cluster", sheet: "Deploy Cluster", slotKey: "mgmt", slotLabel: "Mgmt PG (Deploy Cluster)",
    cells:   { networkTraffic: "D211", name: "D212", lb: "D213", uplink1: "D214", uplink2: "D215" },
    cells90: { networkTraffic: "D199", name: "D200", lb: "D201", uplink1: "D202", uplink2: "D203" } }),
  ..._portgroupSlotEntries({ scope: "additional-cluster", sheet: "Deploy Cluster", slotKey: "vmMgmt", slotLabel: "VM Mgmt PG (Deploy Cluster)",
    cells:   { networkTraffic: "D216", name: "D217", lb: "D218", uplink1: "D219", uplink2: "D220" },
    cells90: { networkTraffic: "D204", name: "D205", lb: "D206", uplink1: "D207", uplink2: "D208" } }),
  ..._portgroupSlotEntries({ scope: "additional-cluster", sheet: "Deploy Cluster", slotKey: "vmotion", slotLabel: "vMotion PG (Deploy Cluster)",
    cells:   { networkTraffic: "D221", name: "D222", lb: "D223", uplink1: "D224", uplink2: "D225" },
    cells90: { networkTraffic: "D209", name: "D210", lb: "D211", uplink1: "D212", uplink2: "D213" } }),
  ..._portgroupSlotEntries({ scope: "additional-cluster", sheet: "Deploy Cluster", slotKey: "principalStorage", slotLabel: "Principal Storage PG (Deploy Cluster)",
    cells:   { networkTraffic: "D226", name: "D227", lb: "D228", uplink1: "D229", uplink2: "D230" },
    cells90: { networkTraffic: "D214", name: "D215", lb: "D216", uplink1: "D217", uplink2: "D218" } }),
  ..._portgroupSlotEntries({ scope: "additional-cluster", sheet: "Deploy Cluster", slotKey: "vsanStorageClient", slotLabel: "vSAN Storage Client PG (Deploy Cluster)",
    cells:   { networkTraffic: "D231", name: "D232", lb: "D233", uplink1: "D234", uplink2: "D235" },
    cells90: { networkTraffic: "D219", name: "D220", lb: "D221", uplink1: "D222", uplink2: "D223" } }),
  ...
  //
  // 9.0 + 9.1 row addresses both confirmed against pristine workbook
  // cell-meta 2026-05-27. Field ordering differs between versions:
  // hostOverlayProfile + uplinkProfileName + numberOfUplinks swap places
  // (9.0 leads the trailing-uplink block with hostOverlayProfile; 9.1
  // leads with uplinkProfileName). cellByVersion handles the offset.
  ...
  ..._nsxHostOverlayBlockEntries({
    ...
    cells90: {
      applyDefault: "D297",
      operationalMode: "D298",
      tzOverlay: "D299", tzVlan: "D300",
      tzName: "D302", vlan: "D303",
      ipAssignment: "D304", staticPoolType: "D305",
      poolName: "D306", poolDescription: "D307",
      cidr: "D308", rangeStart: "D309", rangeEnd: "D310", gatewayIp: "D311",
      vlanTzName: "D312",
      uplinkProfileName: "D318", numberOfUplinks: "D315",
      uplink1Name: "D316", uplink2Name: "D317",
      hostOverlayProfile: "D314",
      teamingPolicy: "D319",
      activeUplink1: "D320", activeUplink2: "D321",
    },
  }),
  ..._nsxHostOverlayBlockEntries({
    scope: "additional-cluster",
    ...
    cells90: {
      applyDefault: "D225",
      operationalMode: "D226",
      tzOverlay: "D227", tzVlan: "D228",
      tzName: "D230", vlan: "D231",
      ipAssignment: "D232", staticPoolType: "D233",
      poolName: "D234", poolDescription: "D235",
      cidr: "D236", rangeStart: "D237", rangeEnd: "D238", gatewayIp: "D239",
      vlanTzName: "D240",
      uplinkProfileName: "D246", numberOfUplinks: "D243",
      uplink1Name: "D244", uplink2Name: "D245",
      hostOverlayProfile: "D242",
      teamingPolicy: "D247",
      activeUplink1: "D248", activeUplink2: "D249",
    },
  }),
  ...
  ..._supervisorConfigureBlockEntries({
    ...
    cells: {
      // 9.1-only fields (the 9.0 workbook did not carry these concepts).
      ...
      cpSize: "D272", apiDns: "D273",
      // Theme N — dual-version (9.0 workbook block at D208-D218).
      version:        { v90: "D208", v91: "D279" },
      edgeCluster:    { v90: "D210", v91: "D281" },
      adminPassword:  { v90: "D212", v91: "D283" },
      node1Ip:        { v90: "D213", v91: "D284" },
      node2Ip:        { v90: "D214", v91: "D285" },
      node3Ip:        { v90: "D215", v91: "D286" },
      clusterVip:     { v90: "D216", v91: "D287" },
      clusterFqdn:    { v90: "D217", v91: "D288" },
      clusterName:    { v90: "D218", v91: "D289" },
    },
  ...
  ..._supervisorConfigureBlockEntries({
    ...
    cells: {
      ...
      cpSize: "D218", apiDns: "D219",
      // Theme N — dual-version (9.0 workbook block at D151-D161).
      version:        { v90: "D151", v91: "D225" },
      edgeCluster:    { v90: "D153", v91: "D227" },
      adminPassword:  { v90: "D155", v91: "D229" },
      node1Ip:        { v90: "D156", v91: "D230" },
      node2Ip:        { v90: "D157", v91: "D231" },
      node3Ip:        { v90: "D158", v91: "D232" },
      clusterVip:     { v90: "D159", v91: "D233" },
      clusterFqdn:    { v90: "D160", v91: "D234" },
      clusterName:    { v90: "D161", v91: "D235" },
    },
  ...
  // to multiple cells.
  {
    sheet: "Deploy Workload Domain", cell: "D341", cellByVersion: { "9.0": "D326", "9.1": "D341" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D342", cellByVersion: { "9.0": "D327", "9.1": "D342" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D343", cellByVersion: { "9.0": "D328", "9.1": "D343" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D344", cellByVersion: { "9.0": "D329", "9.1": "D344" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D349", cellByVersion: { "9.0": "D331", "9.1": "D349" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D350", cellByVersion: { "9.0": "D332", "9.1": "D350" },
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D351", cellByVersion: { "9.0": "D333", "9.1": "D351" },
  ...
  // VPC / Transit Gateway IP-block pools (9.1 structured; ipBlocks dual-version).
  // Two pools × two Configure sheets. Cells verified against the pristine
  // 9.0/9.1 fixtures 2026-06-01. See the VPC/TGW design spec.
  ..._vpcPoolBlockEntries({
    scope: "mgmt-cluster", sheet: "Configure Management Domain",
    poolKey: "externalPool", poolLabel: "VPC External IP Block", scopeShort: "Mgmt",
    ipBlocksVerifyLabel: "VPC External IP Blocks",
    cells: { poolName: "D194", visibility: "D195", ipBlocks: { v90: "D188", v91: "D196" }, excludedIps: "D197", reservedSubnet: "D198" },
  }),
  ..._vpcPoolBlockEntries({
    scope: "mgmt-cluster", sheet: "Configure Management Domain",
    poolKey: "tgwPool", poolLabel: "Private TGW IP Block", scopeShort: "Mgmt",
    ipBlocksVerifyLabel: "Private - Transit Gateway IP Blocks",
    cells: { poolName: "D199", visibility: "D200", ipBlocks: { v90: "D189", v91: "D201" }, excludedIps: "D202", reservedSubnet: "D203" },
  }),
  ..._vpcPoolBlockEntries({
    scope: "workload-cluster", sheet: "Configure Workload Domain",
    poolKey: "externalPool", poolLabel: "VPC External IP Block", scopeShort: "WLD",
    ipBlocksVerifyLabel: "VPC External IP Blocks",
    cells: { poolName: "D137", visibility: "D138", ipBlocks: { v90: "D131", v91: "D139" }, excludedIps: "D140", reservedSubnet: "D141" },
  }),
  ..._vpcPoolBlockEntries({
    scope: "workload-cluster", sheet: "Configure Workload Domain",
    poolKey: "tgwPool", poolLabel: "Private TGW IP Block", scopeShort: "WLD",
    ipBlocksVerifyLabel: "Private - Transit Gateway IP Blocks",
  ...
  {
    sheet: "Deploy Workload Domain", cell: "D353", cellByVersion: { "9.0": "D336", "9.1": "D353" },
  ...
  {
    sheet: "Deploy Management Domain", cell: "L412", cellByVersion: { "9.0": "L84", "9.1": "L412" },
  ...
  {
    sheet: "Deploy Management Domain", cell: "L413", cellByVersion: { "9.0": "L85", "9.1": "L413" },
  ...
  ..._deployNetworkBlock("workload-cluster", "Deploy Workload Domain", "vmotion", "WLD vMotion", {
    ...
    vlan91: "D85", mtu91: "D86", gwCidr91: "D88", ipScheme91: "D84", ipAssignMode91: "D87",
  ...
  ..._deployNetworkBlock("workload-cluster", "Deploy Workload Domain", "vsan", "WLD vSAN", {
    ...
    vlan91: "D96", mtu91: "D97", gwCidr91: "D99", ipScheme91: "D95", ipAssignMode91: "D98",
  ...
  ..._deployNetworkBlock("additional-cluster", "Deploy Cluster", "vmotion", "Additional Cluster vMotion", {
    ...
    vlan91: "D51", mtu91: "D52", gwCidr91: "D54", ipScheme91: "D50", ipAssignMode91: "D53",
  ...
  ..._deployNetworkBlock("additional-cluster", "Deploy Cluster", "vsan", "Additional Cluster vSAN", {
    ...
    networkVerifyLabel: "CIDR Notation", netmaskVerifyLabel: "Netmask",
    vlan91: "D62", mtu91: "D63", gwCidr91: "D65", ipScheme91: "D61", ipAssignMode91: "D64",
  ...
  // IP Scheme (IPv4|IPv6) + IP Assignment (Static|DHCP|SLAAC) for hostTep +
  // edgeTep network pools — 9.1-only. vMotion + vSAN ride the
  // _deployNetworkBlock builder above; hostTep/edgeTep don't, so they get
  // standalone entries. (IP Assignment cell is +3 rows from IP Scheme.)
  ...[
    { sheet: "Deploy Workload Domain", scheme: "D106", assign: "D109", scope: "workload-cluster",   key: "hostTep", label: "WLD Host TEP" },
    { sheet: "Deploy Workload Domain", scheme: "D117", assign: "D120", scope: "workload-cluster",   key: "edgeTep", label: "WLD Edge TEP" },
    { sheet: "Deploy Cluster",         scheme: "D72",  assign: "D75",  scope: "additional-cluster", key: "hostTep", label: "Additional Cluster Host TEP" },
    { sheet: "Deploy Cluster",         scheme: "D83",  assign: "D86",  scope: "additional-cluster", key: "edgeTep", label: "Additional Cluster Edge TEP" },
  ].flatMap(({ sheet, scheme, assign, scope, key, label }) => [
    {
      sheet, cell: scheme, label: `${label} IP Scheme`, verifyLabel: "IP Scheme", workbookVersions: ["9.1"], scope,
      dataValidation: ["IPv4", "IPv6"],
    ...
    {
      sheet, cell: assign, label: `${label} IP Assignment`, verifyLabel: "IP Assignment", workbookVersions: ["9.1"], scope,
      dataValidation: ["Static", "DHCP", "SLAAC"],
      resolve: (f, ctx) => _getClusterNetwork(ctx, key).ipAssignment || "Static",
      apply: (f, ctx, v) => { _ensureClusterNetwork(ctx, key).ipAssignment = ["Static", "DHCP", "SLAAC"].includes(v) ? v : "Static"; },
    },
  ]),
  ...
  {
    // 9.1-only mgmt-cluster dual-stack toggle (coverage sweep). Deploy
    // Management Domain L49 "Dual stack networking". NOTE: unlike the WLD
    // cell above (Include/Exclude), the mgmt cell's dropdown is
    // Selected/Unselected — verified against the pristine 9.1 fixture.
    // Same `cluster.networks.dualStackIpv6` model field.
    sheet: "Deploy Management Domain", cell: "L49",
    label: "Mgmt Dual Stack IPv6 Enabled",
    verifyLabel: "Dual stack networking",
    workbookVersions: ["9.1"],
    scope: "mgmt-cluster",
    dataValidation: ["Selected", "Unselected"],
    resolve: (f, ctx) => {
...
// Fleet-wide structural invariants (VCF-DEPLOYMENT-PATTERNS.md §3). Unlike
// validatePlacementConstraints (VCF-INV-003 only), this walks the whole fleet
// once and emits an issue for every violated invariant. Phase A covers the 8
// rules whose semantics are already pinned by placement-rules.test.js; the
// fixtures all validate clean, so anything emitted here is a real user error.
//
// Each issue: { ruleId, severity: "critical"|"warn", message, instanceId? }.
function validateFleetInvariants(fleet) {
  const issues = [];
  const instances = fleet?.instances || [];
  if (instances.length === 0) return issues;
  const initial = getInitialInstance(fleet);
  const perFleetIds = new Set(
    Object.entries(APPLIANCE_DB)
      .filter(function(pair) { return pair[1].scope === "per-fleet"; })
      .map(function(pair) { return pair[0]; })
  );
  const label = function(id) { return (APPLIANCE_DB[id] && APPLIANCE_DB[id].label) || id; };
  const iname = function(inst) { return inst.name || inst.id; };
  const walk = function(cb) {
    for (const inst of instances)
      for (const dom of inst.domains || [])
        for (const clu of dom.clusters || [])
          for (const e of clu.infraStack || []) cb(inst, dom, clu, e);
  };

  // VCF-INV-001 — exactly one mgmt domain per instance.
  for (const inst of instances) {
    const mgmt = (inst.domains || []).filter(function(d) { return d.type === "mgmt"; });
    if (mgmt.length !== 1) {
      issues.push({ ruleId: "VCF-INV-001", severity: "critical", instanceId: inst.id,
        message: `Instance "${iname(inst)}" must have exactly one management domain (found ${mgmt.length}).` });
    }
  }

  // VCF-INV-010 — per-fleet appliances appear at most once across the fleet
  // (warm-standby replicas excluded via countActivePerFleetEntries).
  for (const id of perFleetIds) {
    const count = countActivePerFleetEntries(fleet, id);
    if (count > 1) {
      issues.push({ ruleId: "VCF-INV-010", severity: "critical",
        message: `${label(id)} is a per-fleet appliance and must appear exactly once across the fleet (found ${count} active).` });
    }
  }

  // VCF-INV-011 — per-fleet appliances live on the initial instance only.
  if (initial) {
    walk(function(inst, dom, clu, e) {
      if (!perFleetIds.has(e.id)) return;
      if (inst.id !== initial.id) {
        issues.push({ ruleId: "VCF-INV-011", severity: "critical", instanceId: inst.id,
          message: `${label(e.id)} (per-fleet) must run on the initial instance "${iname(initial)}", not "${iname(inst)}".` });
      }
    });
  }
  ...
  if (initial) {
    ...
    for (const inst of instances) {
      ...
      let hasNetCollector = false;
      for (const dom of inst.domains || [])
        for (const clu of dom.clusters || [])
          for (const e of clu.infraStack || []) {
            if (e.id === "vcfOpsCollector") hasCollector = true;
            if (e.id === "vcfOpsNetCollector") hasNetCollector = true;
          }
      if (!hasCollector) {
        issues.push({ ruleId: "VCF-INV-012", severity: "critical", instanceId: inst.id,
          message: `Non-initial instance "${iname(inst)}" must deploy a VCF Operations Collector.` });
      }
      if (fleetHasNet && !hasNetCollector) {
        issues.push({ ruleId: "VCF-INV-012", severity: "warn", instanceId: inst.id,
          message: `Non-initial instance "${iname(inst)}" should deploy a VCF Operations for Networks Collector (fleet has Networks enabled).` });
      }
    }
  }

  // VCF-INV-020 — a workload NSX Manager may be shared across workload domains
  // within one instance, never across instances.
  const nsxByKey = new Map();
  walk(function(inst, dom, clu, e) {
    if (e.id !== "nsxMgr" || e.role !== "wld" || !e.key) return;
    if (!nsxByKey.has(e.key)) nsxByKey.set(e.key, new Set());
    nsxByKey.get(e.key).add(inst.id);
  });
  for (const [key, ids] of nsxByKey) {
    if (ids.size > 1) {
      issues.push({ ruleId: "VCF-INV-020", severity: "critical",
        message: `Workload NSX Manager "${key}" is shared across instances ${[...ids].join(", ")}; NSX Manager may only be shared within a single instance.` });
    }
  }

  // VCF-INV-021 — NSX Global Manager only in a federated fleet (>=2 instances
  // AND federation enabled).
  let hasGlobal = false;
  walk(function(inst, dom, clu, e) { if (e.id === "nsxGlobalMgr") hasGlobal = true; });
  if (hasGlobal && (instances.length < 2 || fleet.federationEnabled !== true)) {
    issues.push({ ruleId: "VCF-INV-021", severity: "critical",
      message: `NSX Global Manager requires a federated fleet (>=2 instances with federation enabled); fleet has ${instances.length} instance(s), federationEnabled=${!!fleet.federationEnabled}.` });
  }

  // VCF-INV-040 — a stretched instance (two sites) shares ONE mgmt appliance
  // stack; the SDDC Manager must not be duplicated per site.
  for (const inst of instances) {
    ...
    for (const clu of mgmt.clusters || [])
      for (const e of clu.infraStack || [])
        if (e.id === "sddcMgr") sddc += e.instances || 0;
    if (sddc !== 1) {
      issues.push({ ruleId: "VCF-INV-040", severity: "critical", instanceId: inst.id,
        message: `Stretched instance "${iname(inst)}" must share one management appliance stack (SDDC Manager count must be 1, found ${sddc}).` });
    }
  }

  // VCF-INV-051 — haFederation / haFederationSiteProtection profiles require
  // at least two instances.
  for (const inst of instances) {
    if (!(inst.deploymentProfile || "").includes("Federation")) continue;
    if (instances.length < 2) {
      issues.push({ ruleId: "VCF-INV-051", severity: "critical", instanceId: inst.id,
        message: `Instance "${iname(inst)}" uses a Federation profile (${inst.deploymentProfile}) which requires at least 2 instances; fleet has ${instances.length}.` });
    }
  }

  // VCF-INV-002 — per-instance appliances (and the mgmt role of dual-role
  // appliances like vCenter / NSX Manager) must sit on the instance's mgmt
  // domain. Dual-role 'wld' entries are exempt — they physically run in the
  // mgmt cluster but belong to a workload domain (VCF-APP-003 / INV-003).
  walk(function(inst, dom, clu, e) {
    const def = APPLIANCE_DB[e.id];
    if (!def) return;
    const placedPerInstance = def.scope === "per-instance" || (def.dualRole && e.role === "mgmt");
    if (placedPerInstance && dom.type !== "mgmt") {
      issues.push({ ruleId: "VCF-INV-002", severity: "critical", instanceId: inst.id,
        message: `${label(e.id)} is a per-instance appliance and must run on the management domain of "${iname(inst)}", not a ${dom.type} domain.` });
    }
  });

  // VCF-INV-030 — Identity Broker mode should match fleet size: embedded is
  // for a single instance; multiple instances should use an appliance-based
  // broker (fleet-wide or multi-broker). Soft rule → warn (lab-allowed).
  if ((fleet.ssoMode || inferSsoMode(fleet)) === "embedded" && instances.length > 1) {
    issues.push({ ruleId: "VCF-INV-030", severity: "warn",
      message: `Embedded Identity Broker is intended for a single instance; this fleet has ${instances.length}. Consider an appliance-based broker (fleet-wide or multi-broker) per VCF-SSO-003.` });
  }

  // VCF-INV-031 — soft recommendation of <=5 instances per Identity Broker.
  const brokerStats = ssoInstancesPerBroker(fleet);
  if (brokerStats.overLimit) {
    issues.push({ ruleId: "VCF-INV-031", severity: "warn",
            ...
            clusters: (dom.clusters || []).map(function(cl) {
              ...
              var updated = {
                ...
                networks: (function() {
                  ...
                  // IP Scheme / IP Assignment — backfill defaults on
                  // vmotion/vsan/hostTep/edgeTep when missing (per-network 9.1
                  // cells). mgmt has neither cell, so it's left untouched.
                  for (const k of ["vmotion", "vsan", "hostTep", "edgeTep"]) {
                    if (nets[k] && typeof nets[k] === "object") {
                      if (nets[k].ipScheme === undefined || nets[k].ipScheme === null || nets[k].ipScheme === "") nets[k].ipScheme = "IPv4";
                      if (nets[k].ipAssignment === undefined || nets[k].ipAssignment === null || nets[k].ipAssignment === "") nets[k].ipAssignment = "Static";
                    }
                  }
...
function migrateFleet(raw) {
  ...
  if (preservedVcfVersion) fleet.vcfVersion = preservedVcfVersion;
  {
    ...
    const migratedFleet = {
      ...
      // Whitelist-merge networkConfig against the factory so a v6+
      // import with a partial networkConfig (e.g. `{}` or missing
      // sub-objects) still picks up factory defaults for DNS / NTP /
      // syslog / rootCaBundle. Matches the pattern every other
      // fleet-level config block uses. migrateV5ToV6 already creates
      // the object on V5->V6 paths; this re-asserts the shape for V6+
      // imports that bypass the V5->V6 leg.
      networkConfig: (() => {
        const factory = createFleetNetworkConfig();
        const existing = (fleet.networkConfig && typeof fleet.networkConfig === "object") ? fleet.networkConfig : {};
        ...
        return {
          dns: mergeFlat(factory.dns, existing.dns),
          ntp: mergeFlat(factory.ntp, existing.ntp),
          syslog: mergeFlat(factory.syslog, existing.syslog),
          rootCaBundle: typeof existing.rootCaBundle === "string" || existing.rootCaBundle === null
            ? existing.rootCaBundle
            : factory.rootCaBundle,
        };
      })(),
        ...
        return {
          ...
          domains: (inst.domains || []).map((d) => {
            ...
            const clusters = (d.clusters || []).map((c) => ({
              ...
              // VPC/TGW — backfill cluster.vpcConfig (9.1). Whitelist-merge the
              // flat fields, and recurse one level into the externalPool/tgwPool
              // sub-objects (like supervisorConfig.deployment). Idempotent.
              vpcConfig: (() => {
                const factory = createClusterVpcConfig();
                const existing = (c.vpcConfig && typeof c.vpcConfig === "object") ? c.vpcConfig : {};
                const out = { ...factory };
                for (const k of Object.keys(factory)) {
                  if (k === "externalPool" || k === "tgwPool") continue;
                  if (k in existing && existing[k] !== undefined && existing[k] !== null) out[k] = existing[k];
                }
                for (const poolKey of ["externalPool", "tgwPool"]) {
                  const pf = createVpcIpBlockPool();
                  const pe = (existing[poolKey] && typeof existing[poolKey] === "object") ? existing[poolKey] : {};
                  const po = { ...pf };
                  for (const k of Object.keys(pf)) {
                    if (k in pe && pe[k] !== undefined && pe[k] !== null) po[k] = pe[k];
                  }
                  out[poolKey] = po;
                }
                // capability-tray: derive enabled from data-presence on legacy
