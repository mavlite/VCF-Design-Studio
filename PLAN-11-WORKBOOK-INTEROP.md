...
> **Status:** ✅ Phase 1a + Phase 1b complete (2026-05-21). Phase 1b adds native `.xlsx` export via SheetJS (`emitWorkbookXlsx`), inlined into `vcf-design-studio-v9.html` at build time so the single-HTML offline-use story holds; first-click file-picker modal asks the user to drop the official pristine workbook, parsed copy held in a module-scoped ref for the tab's lifetime (no sessionStorage — Blob isn't JSON-serializable and the 1–2 MB workbook would crowd the 5 MB quota). `Sheet2!J16` version check refuses a mismatched pristine before stamping. Graceful fallback: cancel or wrong-version supplied → cell-map CSV via the modal's "Use Cell Map CSV" button. `detectWorkbookVersion` exported for the upcoming import path. 20 new unit tests on `tests/unit/workbook-xlsx-emitter.test.js` (synthetic-pristine fixture built at runtime — no external .xlsx in CI), full coverage gate green (98.48 stmts / 77.01 branches / 100 funcs / 98.48 lines). SheetJS pinned to 0.20.3 from cdn.sheetjs.com (npm 0.18.5 has known Prototype Pollution + ReDoS CVEs; SheetJS removed npm-publish upstream of 0.20.x). Phase 1.5 human sign-off + Phase 2 (bidirectional import with pre-flight diff) remain.
