...
> **Status:** ✅ Phase 1a complete (2026-05-21). Phase 0 cell-meta fixtures + `WORKBOOK_CELL_MAP` (29 strategic entries covering every scope/version pattern) + CSV emitter + parser + UI button + 25 unit tests + automated `verify-cell-map.mjs` gate are all green. Phase 0 fixtures live in [test-fixtures/workbook/](test-fixtures/workbook/); 1681 entries for 9.0, 1760 for 9.1, both SHA-256 pinned. Re-extraction during Phase 1a corrected the column conventions for "Deploy Workload Domain" and "Deploy Cluster" sheets (B=label, D=value — not C=label, B=value as the original extractor assumed). **Phase 1a corrections**: ESX/vMotion/vSAN VLAN rows shifted ~46 rows up in 9.1 due to inserted VCF Management Network + VCFMS IP Range sub-sections; vCenter detail rows moved L90–L94 → L181–L185; appliance size rows moved into the API-only sub-section at L320–L328. Cell-map `verifyLabel` / `verifyLabelByVersion` overrides handle the cases where the workbook uses a section-header-implied generic label ("VLAN ID", "Appliance Size", "IPv4 address Range From") while the cell-map keeps a semantic label for CSV consumers. Phase 1b (native .xlsx via SheetJS), Phase 1.5 human sign-off, Phase 2 (import) remain.
