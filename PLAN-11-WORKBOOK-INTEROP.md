...
> **Status:** ✅ Phase 1a + Phase 1b + Phase 2 (greenfield) complete (2026-05-21). Phase 2 adds the import direction: "Import Workbook" button accepts either a stamped .xlsx (parsed via SheetJS through `readWorkbookXlsxAsCellMapRows`) or a cell-map CSV (parsed via `parseWorkbookCellMap`); `importWorkbookCellMap(rows)` walks `WORKBOOK_CELL_MAP`, calls each entry's `apply` function, returns a draft fleet + applied/skipped diagnostics; pre-flight confirm modal surfaces detected VCF version + applied count + skipped count + any `computeReconcileDiff` entries before replacing the current fleet (greenfield, not merge). Round-trip emit → CSV/.xlsx → import recovers covered fields end-to-end. 20 new unit tests in `tests/unit/workbook-import.test.js`. Total coverage gate green at 98.23 stmts / 77.39 branches / 100 funcs / 98.23 lines, 1259 unit tests, 17/17 E2E. Phase 1.5 human sign-off + future broadening of `apply` coverage (host FQDNs, network VLANs, VCFMS pool) remain.
