  ...
  return (
    <div className="print-cluster">
      ...
      <div className="print-cluster-identity">
        <h3 className="print-h3 print-keep-with-next">
      ...
      {/* Visual SVG diagrams — reuse the same components the editor renders.
          Browser print handles inline SVG natively at vector resolution. */}
...
function PrintPhysicalTopology({ fleet, fleetResult }) {
  ...
  return (
    <section className="print-page print-section print-landscape">
      ...
      <p className="print-section-intro">
        Rack-level layout showing cluster placement, appliance VMs per cluster,
        and stretched relationships across sites. Rendered landscape so
        the full fleet fits on one page.
      </p>
      {(layout.stretchedBands || []).length > 0 && (
        <div className="print-callout">
          <h4 className="print-h4">Stretched relationships</h4>
          <ul className="print-list">
            {layout.stretchedBands.map((band, i) => (
              <li key={i}>{band.label}</li>
            ))}
          </ul>
        </div>
      )}
      ...
      {(layout.witnesses || []).length > 0 && (
        <div className="print-callout">
          <h4 className="print-h4">vSAN witness sites</h4>
          <ul className="print-list">
            {layout.witnesses.map((w) => (
              <li key={w.id}>{w.label} · {w.size} · {w.instances} VM(s)</li>
            ))}
          </ul>
        </div>
      )}
    ...
    >
      <style>{`
        ...
        @media print {
          ...
          /* Cover page styling — large title + meta block + scope panel. */
          .print-cover { display: flex; flex-direction: column; min-height: 9.5in; }
          .print-cover-header { margin-top: 0.4in; }
          .print-eyebrow {
          ...
          .print-eyebrow-sm {
            ...
            color: #64748b; margin-bottom: 0.5em;
          }
          .print-title {
          ...
          .print-cover-meta th {
            text-align: left; padding: 0.25em 1.5em 0.25em 0;
            color: #64748b; font-weight: 500; vertical-align: top;
            text-transform: uppercase; font-size: 9pt; letter-spacing: 0.1em;
            white-space: nowrap;
          }
          .print-cover-meta td { padding: 0.25em 0; color: #0f172a; font-weight: 500; }
          .print-cover-scope {
            margin-top: 0.5in;
